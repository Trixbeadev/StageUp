...
# Mapeamento: codigo_vaga -> dados corretos
vagas_update = {
    '5859313': {
...
with app.app_context():
    atualizadas = 0
    for codigo_vaga, dados in vagas_update.items():
        vaga = Vaga.query.filter_by(codigo_vaga=codigo_vaga).first()
        
        if vaga:
            # Atualizar dados da vaga existente
            for key, value in dados.items():
                setattr(vaga, key, value)
            db.session.add(vaga)
            atualizadas += 1
            print(f'Atualizada: {dados["titulo"]} ({codigo_vaga})')
        else:
            # Se não existe, criar nova
            vaga = Vaga(codigo_vaga=codigo_vaga, **dados)
            db.session.add(vaga)
            atualizadas += 1
            print(f'Criada: {dados["titulo"]} ({codigo_vaga})')
    
    db.session.commit()
    print(f'\nTotal de vagas atualizadas/criadas: {atualizadas}')
